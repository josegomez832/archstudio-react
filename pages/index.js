import React, {useEffect } from 'react'
import { createClient,contentful } from 'contentful'
import Head from 'next/head'
import Image from 'next/image'
import styles from '../styles/Home.module.css'
import Navigation from '/sections/Header'
import HomeMeetTeam from '../sections/HomeMeetTeam'
import FeaturedPortfolio from '../sections/FeaturedPortfolio'
import Footer from '/sections/Footer'
import Glide from '@glidejs/glide'
import Link from 'next/link'
import AOS from 'aos';
import 'aos/dist/aos.css'; // You can also use <link> for styles
import { AiOutlineArrowRight } from "react-icons/ai"


export async function getStaticProps() {
  
  const client = createClient({
    space: process.env.CONTENTFUL_SPACE_ID,
    accessToken: process.env.CONTENTFUL_ACCESS_KEY,
  })
  const res = await client.getEntries({
    content_type: 'portfolio'
  })

  return {
    props: {
      portfolio: res.items
    }
  }
}

export default function Home({portfolio}) {
  console.log(portfolio);
 
  useEffect(() => {
    // Client-side-only code
    AOS.init();
    new Glide('.glide').mount()
  },[])
  return (
    <div className={styles.container}>
      <Head>
        <title>Arch Studio - High-End Architecture Firm</title>
        <meta name="description" content="Arch Studio - High-End Architecture Firm - Houston, TX" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Navigation />
      
      <main className="container">
      
      <div className="glide section">
        <div className="glide__track" data-glide-el="track">
          <ul className="glide__slides">
            <li className="glide__slide">
              <div className="glide__content">

              <h2>Project Paramour</h2>
              <p>Project made for an art museum near Southwest London. Project Paramour is a statement of bold, modern architecture.</p>
              <Link href="/portfolio">
                <a className='button btn-dark'>See Portfolio <AiOutlineArrowRight /></a>
              </Link>
              </div>
              
              <Image
                src="/arch-slide1.jpg"
                alt="Project Paramour"
                width={1110}
                height={720}
                layout="raw"
              />
            </li>
            <li className="glide__slide">
              <div className="glide__content">

                <h2>Seraph Station</h2>
                <p>The Seraph Station project challenged us to design a unique station that would transport people through time. The result is a fresh and futuristic model inspired by space stations.</p>
                <Link href="/portfolio">
                  <a className='button btn-dark'>See Portfolio <AiOutlineArrowRight /></a>
                </Link>
              </div>
              <Image
                src="/arch-slide2.jpg"
                alt="Project Paramour"
                width={1110}
                height={720}
                layout="raw"
              />
            </li>
            <li className="glide__slide">
            <div className="glide__content">

              <h2>Federal II Tower</h2>
              <p>A sequel theme project for a tower originally built in the 1800s. We achieved this with a striking look of brutal minimalism with modern touches.</p>
              <Link href="/portfolio">
                <a className='button btn-dark'>See Portfolio <AiOutlineArrowRight /></a>
              </Link>
              </div>
              <Image
                src="/arch-slide3.jpg"
                alt="Project Paramour"
                width={1110}
                height={720}
                layout="raw"
              />
            </li>
            <li className="glide__slide">
            <div className="glide__content">

            <h2>Trinity Bank Tower</h2>
            <p>Trinity Bank challenged us to make a concept for a 84 story building located in the middle of a city with a high earthquake frequency. For this project we used curves to blend design and stability to meet our objectives.</p>
            <Link href="/portfolio">
                <a className='button btn-dark'>See Portfolio <AiOutlineArrowRight /></a>
              </Link>
            </div>
              <Image
                src="/arch-slide4.jpg"
                alt="Project Paramour"
                width={1110}
                height={720}
                layout="raw"
              />
            </li>
          </ul>
        </div>
        <div className="glide__buttons" data-glide-el="controls[nav]">
          <button className="glide__bullet" data-glide-dir="=0">1</button>
          <button className="glide__bullet" data-glide-dir="=1">2</button>
          <button className="glide__bullet" data-glide-dir="=2">3</button>
          <button className="glide__bullet" data-glide-dir="=3">4</button>
        </div>
      </div>
      { /*------------- About -------------*/ }
        <div className="about-arch container section" data-aos="fade-in" data-aos-mirror="true">
          <div className="row">
            <div className="col-7">
              <div className="lg-heading">Welcome</div>
              <div className='about-arch--content'>
              <p className="fs-1">Welcome to Arch Studio</p>
              <p>We have a unique network and skillset to help bring your projects to life. Our small team of highly skilled individuals combined with our large network put us in a strong position to deliver exceptional results.</p>
              <p>Over the past 10 years, we have worked on all kinds of projects. From stations to high-rise buildings, we create spaces that inspire and delight.</p>
              <p>We work closely with our clients so that we understand the intricacies of each project. This allows us to work in harmony the surrounding area to create truly stunning projects that will stand the test of time.</p>
              </div>
            </div>
            <div className="col-5 d-md-none">
              <Image
                src="/welcome-img.jpg"
                alt="Welcome to Arch"
                width={350}
                height={568}
                />
            </div>
          </div>
          <div className="row">
           
          </div>
          
        </div>
        { /*------------- End About -------------*/ }
        { /*------------- Meet Team -------------*/ }
        <HomeMeetTeam /> 
        { /*------------- End Meet Team -------------*/ }

        { /*------------- Portfolio -------------*/ }
        <div className='portfolio-section section' data-aos="fade-in" data-aos-mirror="true"> 
          <div className='section-title'>
            <h3>Featured</h3>
            <Link href="/portfolio">
              <a className='button btn-dark'>See All <AiOutlineArrowRight /></a>
            </Link>
          </div>
          <div className='row portfolio-items'>
                { portfolio.map( (project, index) => {
                    let itemCount = index;
                    itemCount++;
                    console.log(itemCount);
                    return (
                      <FeaturedPortfolio key={project.sys.id} project={project} count={itemCount} />
                    )
                }).slice(0,3) }
                
          </div>
        </div> 
        { /*------------- End Portfolio -------------*/ }
      </main>

      <Footer />
    </div>
  )
}
